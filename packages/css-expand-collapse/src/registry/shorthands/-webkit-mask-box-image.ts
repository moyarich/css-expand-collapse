...
const expand: ShorthandExpander = (value, context) => {
  ...
  const stripRepeat = (tokens: string[]) => tokens.filter((token) => {
    if (context.matchProperty(longhandNames[4], token)) {
  ...
  if (sliceTokens.length) {
    ...
    if (!context.matchProperty(longhandNames[1], slice)) return null;
    result[longhandNames[1]] = slice;
  }
  ...
  if (parts[1]) {
    ...
    if (tokens.length) {
      ...
      if (!context.matchProperty(longhandNames[2], width)) return null;
      result[longhandNames[2]] = width;
    }
  }
  ...
  if (parts[2]) {
    ...
    if (tokens.length) {
      ...
      if (!context.matchProperty(longhandNames[3], outset)) return null;
      result[longhandNames[3]] = outset;
    }
  }

  if (repeatTokens.length) {
    const repeat = repeatTokens.join(" ");
    if (!context.matchProperty(longhandNames[4], repeat)) return null;
    result[longhandNames[4]] = repeat;
  }
